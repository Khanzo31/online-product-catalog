...
// Type Definitions (Updated for SEO)
...
async function getProductForSEO(documentId: string): Promise<Product | null> {
...
// --- START OF FIX ---
// 1. Define the type for the props passed to the function
type Props = {
  params: { documentId: string };
};

// 2. Apply the type to the function signature
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  // --- END OF FIX ---
  const product = await getProductForSEO(params.documentId);
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchProductData = async () => {
      ...
      setError(null);
      try {
        const productData = await getProductForSEO(documentId);
        ...
        if (allProductsRes.ok) {
          ...
          if (productData.Product && allProducts.length > 0) {
            const related = allProducts
              .filter(
                (p) =>
                  p.Product?.documentId === productData.Product?.documentId &&
                  p.documentId !== productData.documentId
              )
              .slice(0, 4);
